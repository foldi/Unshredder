  ...
  if (drawState == "load image") {
    ...
    simg.img.loadPixels(); // load pixels
      ...
      for (int a = 0; a < verticalSamples; a++) {
        ...
        totalBlue = 0;
        loc = 0;
        interval = (1000/verticalSamples) * .001;   
        ...
        for (int y = floor(simg.img.height * (interval * a)); y < simg.img.height * (interval * (a + 1)); y++) { // Loop over every row
          ...
          color c = simg.img.pixels[loc]; // get the color at loc
    ...
    for (int i = 0; i < intervals.length; i++) { // loop thru intervals
      if (intervals[i] != 0) {
        ...
        if ((maxDiff - segment.segmentIntervalDiff) < maxDiff / 1.5) { // find the breakpoint bw segments and LCDs
    ...
    float interval = ((1000/verticalSamples) * .001);
...
void loadSourceImage (String url, Boolean local) {
  background(0);
  initialSegments.clear();
...
void placeSegments (int ind, Segment segment) { // recursive
